...
cursor.execute("""SELECT Ag_name, Rat_name, Grade, Outlook, Change, Date_str, Ent_name, Okpo, Ogrn, Inn, Finst, Ag_id, Rat_Sctr, Rat_type, Horizon, Scale_Typer, Currency, Backed_flag INTO [dbo].[Query_b] FROM [dbo].[Base] WHERE Ag_name != 'АМБест Компани' AND Ag_name != 'Интерфакс'""") #выбираем поля в отдельную таблицу
