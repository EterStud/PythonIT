...
cursor.execute("""SELECT Ag_name, Rat_name, Grade, Outlook, Change, Date_str, 
               ...
               Currency, Backed_flag INTO [dbo].[Query_All] 
               FROM [dbo].[Base]
               WHERE Date_str != '01.07.2010' AND Grade !='Приостановлен' 
               AND Grade !='Снят' AND Ag_name != 'АМБест Компани' 
               AND Ag_name != 'Интерфакс' AND Rat_sctr = 'B' AND Scale_typer != 'Isc'""") #выбираем поля в отдельную таблицу
